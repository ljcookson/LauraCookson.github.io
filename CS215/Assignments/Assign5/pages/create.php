    ...
    if(isset($_POST["createButton"]) && $_POST["createButton"]){
        ...
        try {
            ...
            if(!$valid){
                ...
                $createPollQuery = 
                    "BEGIN;
                    INSERT INTO Polls (openDate, closeDate, question, user_ID, createdDate) VALUES ('$open', '$close', '$ques', $userID, NOW());
                    INSERT INTO Answers (answer, poll_ID) VALUES ";
                
                $qnum = 1;
                while($qnum < 6){
                    $qup = $qnum + 1;
                    if(strlen($ans[$qup]) > 0){
                        $createPollQuery .= ("('$ans[$qnum]', LAST_INSERT_ID()), ");
                        $qnum++;
                    } else {
                        $createPollQuery .= ("('$ans[$qnum]', LAST_INSERT_ID())");
                        $qnum = 6;
...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en"> <!--We were directed to validate using XHTML1.1, so I am using the headers from Labs 1 & 2-->
    ...
    <body class="overallContainer">
        ...
        <div class="bottom">
            <a href="https://validator.w3.org/check?uri=http://www.webdev.cs.uregina.ca/~ljc806/Assignments/Assign4/pages/create.php">Validate 'create.php'</a>
        </div>
